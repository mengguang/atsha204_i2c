...
uint8_t atsha204Class::getSerialNumber(uint8_t * response) {
	...
	if (!returnCode)	// should return 0 if successful
	{
		...
		if (!returnCode) { /* Finally if last two reads were successful, read byte 8 of the SN */
...
uint8_t atsha204Class::simpleGetSN(uint8_t *result) {
...
uint8_t atsha204Class::sha204p_receive_response(uint8_t size,
	...
	if (ret_code == SWI_FUNCTION_RETCODE_SUCCESS
			|| ret_code == SWI_FUNCTION_RETCODE_RX_FAIL) {
...
uint8_t atsha204Class::sha204c_send_and_receive(uint8_t *tx_buffer,
	...
	uint16_t execution_timeout_us = (uint16_t) (execution_timeout * 1000)
			+ SHA204_RESPONSE_TIMEOUT;
	...
	while ((n_retries_send-- > 0) && (ret_code != SHA204_SUCCESS)) {
		...
		while (n_retries_receive-- > 0) {
			...
			} while ((timeout_countdown > SHA204_RESPONSE_TIMEOUT)
...
uint8_t atsha204Class::sha204m_read(uint8_t *tx_buffer, uint8_t *rx_buffer,
	...
	rx_size = (zone & SHA204_ZONE_COUNT_FLAG) ?
	READ_32_RSP_SIZE :
												READ_4_RSP_SIZE;
